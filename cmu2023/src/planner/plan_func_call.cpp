...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception(fmt::format("func call {} need only one args", func_name));
  }
  StringExpressionType ty = StringExpressionType::Upper;
  if (func_name == "lower") {
    ty = StringExpressionType::Lower;
  }
  std::shared_ptr<AbstractExpression> res = std::make_shared<StringExpression>(args[0], ty);
  return res;
}
